...
from ..md_edit import MetadataEditable

class EditorInterface:
    """Handles the UI for track editing"""
    def __init__(self, library, md_editor, input_handler):
    ...
    def edit_track(self, track_key: str) -> None:
        ...
        while True:
            ...
            choice = self.input_handler.prompt_choose(
                "Apply these changes?",
                displayed_options=["Yes", "Redo", "Quit"],
                inputs=["y", "r", "q"],
            )

            if choice == "Yes":
                # Update both in-memory and file metadata
                self.library.update_metadata(track_key, new_values)
